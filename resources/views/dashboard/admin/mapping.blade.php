    ...
    <div class="max-w-[1400px] mx-auto w-full" x-data="{
        ...
        <div x-show="viewMode === 'list'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-10 gap-6">
                <div>
                    <h2 class="font-headline font-black text-3xl text-primary tracking-tighter uppercase leading-none">Manajemen Alokasi</h2>
                    <div class="flex flex-wrap gap-4 mt-6">
                        <button @click="activeTab = 'priority'" 
                            ...
                            🚨 Darurat (Stok 0)
                            <span class="ml-2 px-1.5 py-0.5 bg-white text-red-600 text-[8px] font-black border border-red-600">3</span>
                        </button>
                        <button @click="activeTab = 'optimize'" 
                            ...
                            <span class="ml-2 px-1.5 py-0.5 bg-white text-amber-600 text-[8px] font-black border border-amber-600">2</span>
                        </button>
            ...
            <div x-show="activeTab === 'priority'" x-transition class="space-y-6">
                <div class="bg-red-50 border-[3px] border-red-200 p-4 flex items-center gap-4 mb-6">
                    ...
                    <p class="text-[10px] font-bold text-red-800 uppercase tracking-widest">Reseller di bawah memiliki Distributor yang stoknya HABIS. Alihkan ke Distributor lain segera!</p>
                </div>

                <div class="grid grid-cols-1 gap-4">
                    @php
                        $priorityResellers = [
                            ['id' => 101, 'name' => 'Ahmad Fauzi', 'city' => 'Bandung', 'old_dist' => 'PT. Maju Logistik', 'dist_city' => 'Bandung', 'stock' => 0],
                            ['id' => 102, 'name' => 'Toko Sinar Jaya', 'city' => 'Cimahi', 'old_dist' => 'PT. Maju Logistik', 'dist_city' => 'Bandung', 'stock' => 0],
                            ['id' => 103, 'name' => 'Budi Subroto', 'city' => 'Bandung', 'old_dist' => 'PT. Maju Logistik', 'dist_city' => 'Bandung', 'stock' => 0],
                        ];
                    @endphp

                    @foreach($priorityResellers as $reseller)
                    <div class="bg-white border-[3px] border-gray-900 shadow-[6px_6px_0_var(--color-red-600)] p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-red-50 transition-colors">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-red-600 flex items-center justify-center border-2 border-gray-900 text-white font-headline font-black">!</div>
                            <div>
                                <h4 class="font-headline font-black text-lg text-gray-900 uppercase tracking-tight">{{ $reseller['name'] }}</h4>
                                <div class="flex gap-2 items-center">
                                    <p class="text-[10px] font-bold text-red-600 uppercase tracking-widest">Gudang: {{ $reseller['old_dist'] }}</p>
                                    <span class="text-[8px] bg-red-100 text-red-700 px-1.5 py-0.5 font-black uppercase">{{ $reseller['dist_city'] }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-6 w-full md:w-auto">
                            <div class="text-right hidden lg:block">
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Wilayah Reseller</p>
            ...
            <div x-show="activeTab === 'optimize'" x-transition style="display: none;">
                <div class="bg-amber-50 border-[3px] border-amber-200 p-4 flex items-center gap-4 mb-6">
                    <svg class="w-6 h-6 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                    <p class="text-[10px] font-bold text-amber-800 uppercase tracking-widest">Reseller ini dilayani oleh Distributor LUAR KOTA. Sekarang sudah ada Distributor LOKAL di wilayah mereka.</p>
                </div>

                <div class="grid grid-cols-1 gap-4">
                    @php
                        $optimizeResellers = [
                            ['id' => 104, 'name' => 'Citra Mandiri', 'city' => 'Bekasi', 'old_dist' => 'Indo Cipta Chem', 'dist_city' => 'Cirebon', 'stock' => 800],
                            ['id' => 105, 'name' => 'Toko Barokah', 'city' => 'Bekasi', 'old_dist' => 'Indo Cipta Chem', 'dist_city' => 'Cirebon', 'stock' => 800],
                        ];
                    @endphp

                    @foreach($optimizeResellers as $reseller)
                    <div class="bg-white border-[3px] border-gray-900 shadow-[6px_6px_0_var(--color-amber-500)] p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:bg-amber-50 transition-colors">
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-amber-500 flex items-center justify-center border-2 border-gray-900 text-white font-headline font-black">
                ...
                <div class="bg-white border-[4px] border-gray-900 shadow-[8px_8px_0_var(--color-primary-darkest)] overflow-hidden">
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-gray-900 text-white">
                            <tr>
                                <th class="px-6 py-4 text-[10px] font-headline font-bold uppercase tracking-widest">Mitra / Reseller</th>
                                <th class="px-6 py-4 text-[10px] font-headline font-bold uppercase tracking-widest text-center">Wilayah Mitra</th>
                                <th class="px-6 py-4 text-[10px] font-headline font-bold uppercase tracking-widest">Distributor Aktif</th>
                        ...
                        <tbody class="divide-y-2 divide-neutral-border">
                            @php
                                $allResellers = [
                                    ['id' => 106, 'name' => 'Indah Chem', 'city' => 'Cirebon', 'old_dist' => 'Indo Cipta Chem', 'dist_city' => 'Cirebon', 'stock' => 800],
                                    ['id' => 107, 'name' => 'Karya Berkah', 'city' => 'Surabaya', 'old_dist' => 'Sentra Logistik Jatim', 'dist_city' => 'Sidoarjo', 'stock' => 2500],
                                ];
                            @endphp
                            ...
                            <tr class="hover:bg-neutral-light transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <p class="text-[9px] font-bold text-slate-500 uppercase tracking-widest">ID: #{{ $reseller['id'] }}</p>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <span class="bg-neutral-light border-2 border-gray-900 px-2 py-1 text-[9px] font-bold text-gray-900 uppercase">{{ $reseller['city'] }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="text-xs font-bold text-primary uppercase">{{ $reseller['old_dist'] }}</p>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Lokasi: {{ $reseller['dist_city'] }}</span>
                                        <span class="text-[9px] text-slate-300">|</span>
                                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Stok: {{ $reseller['stock'] }}</span>
                                    </div>
                                </td>
        ...
        <div x-show="viewMode === 'detail'" style="display: none;" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0">
            ...
            <div class="mb-6 flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
                <div>
                    <h2 class="font-headline font-black text-2xl text-primary tracking-tighter uppercase">Eksekusi Perpindahan</h2>
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Sistem Pemetaan Wilayah CeeKlin</p>
                </div>
            ...
            <div class="bg-white border-[6px] border-gray-900 shadow-[12px_12px_0_var(--color-secondary)] w-full max-w-2xl flex flex-col mx-auto overflow-hidden">
                
                {{-- Form Migrasi --}}
                <div x-show="!migrationSuccess" x-transition>
                    <form action="/dashboard/admin/mapping/migrate" method="POST" @submit.prevent="submitMigration">
                        @csrf
                        ...
                        <div class="p-6 md:p-8 bg-neutral flex flex-col gap-6">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div class="bg-white border-[3px] border-gray-900 p-4 shadow-[4px_4px_0_rgba(0,0,0,0.1)]">
                                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Pihak Reseller</p>
                                    <p class="font-headline font-black text-base text-gray-900 uppercase" x-text="selectedReseller?.name"></p>
                                    <p class="text-[10px] font-bold text-primary uppercase" x-text="selectedReseller?.city"></p>
                                </div>
                                <div class="bg-white border-[3px] border-gray-900 p-4 shadow-[4px_4px_0_rgba(0,0,0,0.1)]">
                                    <p class="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Distributor Saat Ini</p>
                                    <p class="font-headline font-black text-base text-red-600 uppercase" x-text="oldDistributorName"></p>
                                    <p class="text-[10px] font-bold text-slate-500 uppercase" x-text="selectedReseller?.dist_city"></p>
                                </div>
                            </div>

                            <div class="flex flex-col gap-2">
                                <label class="text-[10px] font-bold text-gray-900 uppercase tracking-widest flex items-center gap-2">
                                ...
                                <div class="relative">
                                    <select name="distributor_id" x-model="targetDistributorId" required
                                        ...
                                        {{-- Skenario 1 & 3: Emergency & Semua (Sort by Proximity & Stock) --}}
                                        <template x-if="activeTab === 'priority' || activeTab === 'all'">
                                            <optgroup label="REKOMENDASI TERDEKAT & STOK TERBANYAK">
                                                {{-- BACKEND-TODO: Sort by Proximity to Reseller's City, then by Stock DESC --}}
                                                {{-- Jika di wilayah yang sama tidak ada stok, tampilkan dari provinsi terdekat --}}
                                                <option value="4">Pusat Distribusi Bandung [BANDUNG] — Stok: 5.000</option>
                                                <option value="2">Teknik Karya Supply [BEKASI] — Stok: 1.250</option>
                                                <option value="3">Indo Cipta Chem [CIREBON] — Stok: 800</option>
                                            </optgroup>
                                        </template>
                                        ...
                                        <template x-if="activeTab === 'optimize'">
                                            <optgroup :label="'DISTRIBUTOR LOKAL DI ' + (selectedReseller?.city || '')">
                                                {{-- BACKEND-TODO: Filter ONLY where city_id === reseller.city_id --}}
                                                <option value="5">CeeKlin Lokal Bekasi [BEKASI] — Stok: 3.200</option>
                                                <option value="6">Mitra Sejahtera Bekasi [BEKASI] — Stok: 1.100</option>
                                            </optgroup>
                                        </template>
                                ...
                                <p x-show="activeTab === 'priority'" class="text-[9px] font-bold text-red-600 uppercase mt-1 tracking-tight">
                                    * Menampilkan rekomendasi distributor terdekat dengan stok mencukupi.
                                </p>
                                <p x-show="activeTab === 'optimize'" class="text-[9px] font-bold text-amber-600 uppercase mt-1 tracking-tight">
                ...
                <div x-show="migrationSuccess" x-transition style="display:none;" class="p-6 md:p-10 flex flex-col items-center text-center">
                    <div class="w-16 h-16 bg-secondary flex items-center justify-center border-[3px] border-gray-900 shadow-[4px_4px_0_var(--color-primary-darkest)] mb-6">
                    ...
                    <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-8">Informasikan semua pihak agar jalur distribusi kembali normal:</p>
                    ...
                    <div class="w-full max-w-md flex flex-col gap-4">
                        ...
                        <div class="flex items-center gap-4 bg-neutral-light border-[3px] border-gray-900 p-4 shadow-[4px_4px_0_rgba(0,0,0,0.05)] transition-all"
                            ...
                            <div class="flex-1 text-left">
                                <p class="text-[9px] font-black uppercase tracking-widest mb-0.5" :class="notified.newDist ? 'text-green-700' : 'text-slate-400'">Distributor Baru</p>
                        ...
                        <div class="flex items-center gap-4 bg-neutral-light border-[3px] border-gray-900 p-4 shadow-[4px_4px_0_rgba(0,0,0,0.05)] transition-all"
                            ...
                            <div class="flex-1 text-left">
                                <p class="text-[9px] font-black uppercase tracking-widest mb-0.5" :class="notified.oldDist ? 'text-green-700' : 'text-slate-400'">Distributor Lama</p>
                        ...
                        <button @click="goBack()" class="mt-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-primary transition-colors underline underline-offset-4">
