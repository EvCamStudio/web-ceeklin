<x-layouts.dashboard bgTheme="dark">
    ...
    <div class="max-w-[1400px] mx-auto w-full relative" x-data="{ 
        ...
        <div class="bg-white border-[4px] border-gray-900 shadow-[8px_8px_0_var(--color-primary-darkest)]" x-show="viewMode === 'list'">
            ...
            {{-- BACKEND-TODO: Loop data --}}
            <div class="divide-y-2 divide-neutral-border">
                @php
                $orders = [
                    ['id' => 'ORD-1092', 'qty' => 5000, 'total' => 'Rp 65.000.000', 'status' => 'Selesai', 'date' => '12 Apr 2026', 'statusClass' => 'border-green-600 text-green-700 bg-green-50', 'leftBorder' => 'border-green-600', 'tracking' => 'RESI-882910', 'courier' => 'Internal CeeKlin', 'notes' => 'Barang sudah sampai di gudang distributor.'],
                    ['id' => 'ORD-1088', 'qty' => 2000, 'total' => 'Rp 26.000.000', 'status' => 'Dikirim', 'date' => '5 Apr 2026', 'statusClass' => 'border-blue-500 text-blue-700 bg-blue-50', 'leftBorder' => 'border-blue-500', 'tracking' => 'RESI-771122', 'courier' => 'Lalamove', 'notes' => 'Dalam perjalanan menuju Cikarang.'],
                    ['id' => 'ORD-1085', 'qty' => 3000, 'total' => 'Rp 39.000.000', 'status' => 'Dikemas', 'date' => 'Kemarin', 'statusClass' => 'border-yellow-500 text-yellow-700 bg-yellow-50', 'leftBorder' => 'border-yellow-500', 'tracking' => '', 'courier' => '', 'notes' => 'Pesanan sedang disiapkan di pabrik.'],
                ];
                @endphp

                @foreach($orders as $order)
                <div class="flex flex-col md:grid md:grid-cols-12 gap-4 px-6 py-5 items-start md:items-center border-l-[5px] {{ $order['leftBorder'] }} hover:bg-neutral-light/50 transition-colors duration-150 group">
                    {{-- No. Order --}}
                    <div class="md:col-span-2 w-full">
                        ...
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $order['date'] }}</p>
                    </div>
                    {{-- Volume --}}
                    <div class="md:col-span-2 w-full flex justify-between md:block md:text-right">
                        ...
                        <p class="font-headline font-black text-xl text-primary tracking-tighter italic">{{ number_format($order['qty'], 0, ',', '.') }} <span class="text-[10px] font-body font-bold text-slate-400">PCS</span></p>
                    </div>
                    {{-- Total --}}
                    <div class="md:col-span-2 w-full flex justify-between md:block md:text-right">
                        <p class="md:hidden text-[8px] font-bold text-slate-400 uppercase tracking-widest">Total Bayar</p>
                        <p class="font-bold text-sm text-gray-900 italic">{{ $order['total'] }}</p>
                    </div>
                    {{-- Status --}}
                    <div class="md:col-span-3 w-full flex justify-between md:block md:text-center">
                        ...
                        <div class="flex flex-col md:items-center">
                            <span class="px-2 py-1 border-2 {{ $order['statusClass'] }} text-[9px] font-black uppercase tracking-widest block w-max shadow-[2px_2px_0_rgba(0,0,0,0.05)]">{{ $order['status'] }}</span>
                        </div>
                    </div>
                    {{-- Aksi --}}
                    <div class="md:col-span-3 w-full flex justify-start md:justify-end">
                        <button @click="openDetail({{ json_encode($order) }})"
                ...
                {{-- Hidden Orders (Expanded) --}}
                <template x-if="showAll">
                    <div>
                        @php
                        $olderOrders = [
                            ['id' => 'ORD-1074', 'qty' => 1000, 'total' => 'Rp 13.000.000', 'status' => 'Selesai', 'date' => '10 Mar 2026', 'statusClass' => 'border-green-600 text-green-700 bg-green-50', 'leftBorder' => 'border-green-600', 'tracking' => 'RESI-123456', 'courier' => 'Internal', 'notes' => 'Selesai.'],
                        ];
                        @endphp
                        @foreach($olderOrders as $order)
                        <div class="flex flex-col md:grid md:grid-cols-12 gap-4 px-6 py-5 items-start md:items-center border-l-[5px] {{ $order['leftBorder'] }} bg-neutral-light/20 hover:bg-neutral-light/50 transition-colors duration-150 group">
                            <div class="md:col-span-2 w-full">
                                <p class="md:hidden text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-1">No. Order</p>
                                <p class="font-headline font-black text-sm text-gray-900 tracking-tight">{{ $order['id'] }}</p>
                                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{{ $order['date'] }}</p>
                            ...
                            <div class="md:col-span-3 w-full flex justify-between md:block md:text-center">
                                <p class="md:hidden text-[8px] font-bold text-slate-400 uppercase tracking-widest">Status</p>
                                <div class="flex flex-col md:items-center">
                                    <span class="px-2 py-1 border-2 {{ $order['statusClass'] }} text-[9px] font-black uppercase tracking-widest block w-max">{{ $order['status'] }}</span>
                                </div>
                            </div>
                            <div class="md:col-span-3 w-full flex justify-start md:justify-end">
                                <button @click="openDetail({{ json_encode($order) }})"
