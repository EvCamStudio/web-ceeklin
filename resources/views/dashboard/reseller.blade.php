<x-layouts.dashboard bgTheme="dark">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-white border-[3px] border-primary shadow-[6px_6px_0_var(--color-primary-darkest)] p-6 flex flex-col justify-between">
            <div>
                <p class="text-[10px] text-secondary font-bold uppercase tracking-widest mb-1 italic">Total Saldo Bonus Aktif</p>
                <h3 class="font-headline font-black text-3xl md:text-4xl text-primary tracking-tighter italic mb-2">Rp {{ number_format($user->bonus_balance ?? 0, 0, ',', '.') }}</h3>
                <div class="flex flex-wrap gap-x-4 gap-y-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                    <span>Referral: <span class="text-gray-900">Rp {{ number_format($user->referral_bonus ?? 0, 0, ',', '.') }}</span></span>
                    <span>Target: <span class="text-gray-900">Rp {{ number_format($user->target_bonus ?? 0, 0, ',', '.') }}</span></span>
                </div>
            </div>
            <div class="mt-6">
        ...
        <div class="bg-white border-[3px] border-secondary shadow-[6px_6px_0_var(--color-gray-900)] p-6 flex flex-col justify-between">
            <div>
                <div class="flex justify-between items-start mb-2">
                    ...
                    <span class="bg-primary text-white text-[9px] font-bold px-2 py-0.5 uppercase border border-gray-900 italic">Hadiah: Rp {{ number_format($targetReward, 0, ',', '.') }}</span>
                </div>
                <h3 class="font-headline font-black text-2xl md:text-3xl text-gray-900 tracking-tighter mb-4 italic">
                    {{ number_format($currentMonthOrders) }} <span class="text-lg text-slate-400">/ {{ number_format($targetQty) }} PCS</span>
                </h3>
                ...
                <p class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                    <span class="text-secondary">{{ number_format($neededForTarget) }} pcs</span> lagi untuk capai target bulan ini!
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 bg-white border-[4px] border-gray-900 shadow-[8px_8px_0_var(--color-primary-darkest)]">
            <div class="bg-primary px-6 py-3">
                <span class="font-headline font-black text-white text-base uppercase tracking-tight italic">Distributor Anda</span>
            </div>
            <div class="p-6 grid grid-cols-1 sm:grid-cols-3 gap-6 items-start">
                <div class="flex flex-col gap-1">
                    <p class="text-[10px] text-primary font-bold uppercase tracking-widest italic">Wilayah</p>
                    <p class="font-headline font-black text-primary text-lg italic uppercase">{{ $upline->city_name ?? 'Pusat' }}</p>
                </div>
                <div class="flex flex-col gap-1">
                    <p class="text-[10px] text-primary font-bold uppercase tracking-widest italic">Distributor</p>
                    <p class="font-bold text-gray-900 text-sm uppercase">{{ $upline->name ?? 'Pusat' }}</p>
                    <p class="text-[10px] text-slate-400 uppercase tracking-widest italic">{{ $upline->province_name ?? '' }} Hub</p>
                </div>
                <div>
                    ...
                    @else
                    <p class="text-[10px] font-bold text-slate-400 uppercase italic">Kontak tidak tersedia</p>
                    @endif
                </div>
            </div>
        </div>
