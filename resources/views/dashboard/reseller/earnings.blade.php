        ...
        <div class="xl:col-span-2 bg-white border-[4px] border-gray-900 shadow-[8px_8px_0_var(--color-primary-darkest)]">
            ...
            <div class="p-6">
                ...
                <div class="h-[180px] relative border-l-2 border-b-2 border-neutral-border">
                    ...
                    <div class="absolute bottom-2 left-0 right-0 flex justify-between px-2 gap-1">
                        @foreach(['Mei','Jun','Jul','Agu','Sep','Okt','Nov'] as $bln)
                        <span class="text-[7px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-tight sm:tracking-widest">{{ $bln }}</span>
                        @endforeach
                    </div>
                </div>
            </div>
            ...
            <div class="divide-y-2 divide-neutral-border">
                ...
                <div class="flex items-start justify-between px-5 py-4 gap-4">
                    <div class="flex-1 min-w-0">
                        <p class="font-bold text-[11px] sm:text-xs text-gray-900 truncate leading-tight uppercase">{{ $tx['nama'] }}</p>
                        <p class="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">{{ $tx['ket'] }}</p>
                        <p class="text-[9px] sm:text-[10px] text-slate-400 mt-1">{{ $tx['tgl'] }}</p>
                    </div>
                    <span class="font-headline font-black text-primary text-xs sm:text-sm flex-shrink-0">{{ $tx['nominal'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
