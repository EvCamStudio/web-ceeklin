    ...
    <form action="{{ route('reseller.order.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start" x-data="{ 
        ...
        <div class="lg:col-span-2 flex flex-col gap-6">
            ...
            <div class="bg-white border-[4px] border-gray-900 shadow-[8px_8px_0_var(--color-primary-darkest)]">
                <div class="bg-primary px-6 py-3 border-b-[4px] border-gray-900">
                    <span class="font-headline font-black text-white text-base uppercase tracking-tight italic">Produk & Kuantitas</span>
                </div>
                    ...
                    <div class="border-[3px] border-primary p-4 md:p-5 flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-5 hover:border-secondary transition-colors duration-150">
                        <div class="flex items-center gap-4 w-full md:w-auto">
                            <div class="w-14 h-14 md:w-16 md:h-16 bg-gray-900 border-2 border-gray-900 flex items-center justify-center flex-shrink-0">
                                <img src="{{ asset('images/hero-bottle.jpeg') }}" alt="CeeKlin 450ml" class="w-full h-auto mix-blend-screen opacity-80">
                            </div>
                            <div class="flex-1">
                                <h4 class="font-headline font-black text-primary uppercase text-sm leading-tight italic">CeeKlin 450ml</h4>
                                <p class="text-[10px] md:text-xs text-slate-500 mt-0.5 uppercase font-bold italic">Jagonya Pembasmi Noda Pakaian</p>
                            </div>
                        </div>
                        
                        <div class="flex items-center justify-between w-full md:contents italic">
                            <div class="md:flex-1">
                                <p class="font-headline font-black text-base md:text-lg text-primary italic" x-text="formatRupiah(price)">Rp 20.000 <span class="hidden md:inline text-xs text-slate-400 font-normal">/ pcs</span></p>
                            </div>
                            <div class="flex items-center gap-2 flex-shrink-0">
                                <button type="button" @click="if(qty > 50) qty--" aria-label="Kurangi jumlah"
                                    class="w-10 h-10 md:w-8 md:h-8 border-[2px] border-primary text-primary flex items-center justify-center font-bold hover:bg-primary hover:text-white transition-colors text-xl md:text-base italic">−</button>
                                <input type="number" x-model.number="qty" min="50" aria-label="Jumlah Pesanan"
                                    class="w-20 md:w-24 text-center bg-neutral-light border-b-2 border-primary py-1 font-headline font-bold text-base md:text-lg focus:outline-none italic">
            ...
            <div class="bg-white border-[4px] border-gray-900 shadow-[4px_4px_0_var(--color-gray-900)]">
                <div class="bg-gray-900 px-6 py-3 border-b-[4px] border-gray-900">
                    <span class="font-headline font-black text-white text-sm uppercase tracking-tight italic">Distributor Anda</span>
                </div>
                <div class="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div class="border-l-[5px] border-secondary pl-4">
                        <p class="font-bold text-sm text-gray-900 uppercase italic">{{ $upline->name }}</p>
                        <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5 italic">{{ $upline->city_name ?? 'Wilayah Distribusi' }} Hub</p>
                    </div>
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $upline->phone) }}" target="_blank"
                        class="w-full sm:w-auto bg-secondary text-white px-4 py-2 font-headline font-bold text-[10px] uppercase tracking-widest border-2 border-gray-900 shadow-[3px_3px_0_var(--color-gray-900)] hover:bg-secondary-dark active:translate-y-0.5 active:shadow-none transition-all flex items-center justify-center gap-2 italic">
        ...
        <div class="bg-white border-[4px] border-gray-900 shadow-[8px_8px_0_var(--color-secondary)] sticky top-6">
            <div class="bg-secondary px-6 py-3 border-b-[4px] border-gray-900">
                <span class="font-headline font-black text-white text-base uppercase tracking-tight italic">Ringkasan Pesanan</span>
            </div>
            <div class="p-6">
                <div class="flex flex-col gap-2 border-b-2 border-neutral-border pb-4 mb-4 text-sm italic">
                    <div class="flex justify-between">
                        <span class="text-slate-500 uppercase font-bold text-[10px]">CeeKlin 450ml × <span x-text="Math.max(50, qty)"></span></span>
                        <span class="font-bold text-gray-900" x-text="formatRupiah(total)"></span>
                    </div>
                </div>
                <div class="flex flex-col gap-2 border-b-2 border-neutral-border pb-4 mb-4 text-sm italic">
                    <div class="flex justify-between">
                        <span class="text-slate-500 uppercase font-bold text-[10px]">Ongkos Kirim</span>
                        <span class="font-bold text-secondary">Ditentukan Distributor</span>
                    </div>
                </div>
                <div class="flex justify-between items-center mb-5 italic">
                    <span class="font-headline font-black text-primary uppercase text-sm tracking-tight italic">Total Bayar</span>
                    <span class="font-headline font-black text-primary text-2xl tracking-tighter italic" x-text="formatRupiah(total)"></span>
                </div>
                ...
                <div class="bg-secondary/10 border-[2px] border-secondary p-3 mb-5 flex items-center gap-2" :class="qty >= 50 ? 'bg-secondary/10 border-secondary' : 'bg-red-50 border-red-500'">
                    ...
                    <p class="text-[10px] font-bold uppercase tracking-widest italic" :class="qty >= 50 ? 'text-secondary' : 'text-red-500'" x-text="qty >= 50 ? 'Minimum order 50pcs terpenuhi ✓' : 'Minimum order 50pcs belum terpenuhi!'"></p>
                </div>
                <button type="submit" aria-label="Kirim pesanan ke distributor" :disabled="qty < 50"
                    class="w-full bg-primary text-white py-4 font-headline font-black text-sm uppercase tracking-widest border-[3px] border-gray-900 shadow-[4px_4px_0_var(--color-gray-900)] hover:bg-primary-hover active:translate-y-0.5 active:shadow-none disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 italic">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    Buat Pesanan Sekarang
                </button>
                <p class="text-[10px] text-slate-400 text-center mt-3 uppercase tracking-widest font-bold italic">Order akan diverifikasi oleh distributor</p>
            </div>
        </div>
    </form>
    </div>
